...
- Fleet and Monitor renew a short authenticated operational-observer lease while actively viewing a gateway. The lease activates the gateway's shared low-priority operational cursor, while Flasher's separate ten-second configured-peer inventory cursor progressively refreshes maintenance metadata; both stop when the view or connection disappears.
- MQTT gateway selection is now remembered per broker host, port, topic root, and username. Flasher restores the gateway previously used with that broker; if it is unavailable and exactly one gateway is discovered, that sole gateway is selected automatically. Multiple unmatched gateways still require an explicit choice, and unavailable targets cannot receive commands that would only time out.
...
- Fleet reads the selected gateway's existing peer cache as soon as its USB or MQTT session becomes usable, so configured remotes appear immediately without an operator refresh action. Address-range candidate discovery remains separate from the observed configured-peer refresh path.
