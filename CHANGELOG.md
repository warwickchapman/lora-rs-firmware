...
- Relay control now owns the complete command and ACK-recovery window: MQTT relay commands are queued into the control-first scheduler with bounded retry delays of 500 ms, 1.5 s, and 3 s, lower-priority polling, maintenance, scans, diagnostics, and management handoffs remain paused until the correlated transaction settles, and missing group ACKs continue to receive bounded idempotent direct `Change` retries.
- Periodic remote sensor reporting now queues sensor pages only instead of unnecessarily sending identity and version pages on every sensor interval. Remote transaction ACKs and input changes are also serviced before pending low-priority maintenance pages.
- Removed the per-peer MQTT `poll_interval_s` command and its firmware state. Configuration schema 6 replaces the old MQTT-specific polling settings outright with `remote_refresh_enabled` and `remote_refresh_cycle_ms`, which describe the gateway-wide headless fleet cycle without compatibility aliases.
...
### Flasher Features
- Fleet and Monitor now progressively hydrate missing remote identity, firmware, WiFi/IP, and uptime once per observed gateway session over both Serial and MQTT. Each incomplete remote receives at most one automatic low-priority maintenance request; unreachable remotes do not create a recurring sweep, and **Scan** remains the explicit retry action.
- Fleet now activates and refreshes the selected MQTT gateway automatically, just as it does over USB Serial; the redundant **Load gateway** button has been removed. MQTT and Serial now hydrate the same bounded one-peer cache record for firmware, role, WiFi/IP, uptime, relay, input, and RSSI without a whole-fleet payload; MQTT sensors continue through their existing bounded per-sensor topics.
- Retained MQTT messages now populate cached values without falsely resetting a remote's Age. Only live MQTT delivery or a gateway peer detail with an explicit age is treated as fresh evidence.
- After an explicit Fleet scan, Flasher now hydrates every cached detail row and sends one bounded maintenance retry only to remotes still missing Identity or Version data. A merely unhydrated UI row no longer causes duplicate RF traffic.
- Fleet and Monitor now renew a short authenticated operational-observer lease while actively viewing a gateway. Their five-second UI cache reads remain RF-free; the lease simply activates the gateway's shared low-priority operational cursor and expires automatically when the view or connection disappears.
- MQTT gateway selection is now remembered per broker host, port, topic root, and username. Flasher restores the gateway previously used with that broker; if it is unavailable and exactly one gateway is discovered, that sole gateway is selected automatically. Multiple unmatched gateways still require an explicit choice, and unavailable targets cannot receive Load/Scan commands that would only time out.
- Replaced the ambiguous large/small connection dots with labelled **S** (Serial) and **M** (MQTT) status tags. The selected transport is bold, the available background transport is muted, and colour distinguishes ready, connecting, disconnected, and failed states without presenting an unused disconnected broker as an error.
- Restored Gateway Session transports now activate automatically after startup state hydration: Remote MQTT immediately connects with the saved non-secret broker parameters, and Local MQTT immediately starts and connects. Changing transport uses the same activation path, so opening Connection settings and pressing Connect is only a failure-recovery or editing step.
- Fleet now reads the selected gateway's existing peer cache as soon as its USB or MQTT session becomes usable, so configured remotes appear without clicking Scan. This initial enumeration and ongoing cache polling do not start a LoRa scan or request per-peer LoRa refreshes; Scan remains the explicit control for probing remotes and candidates.
