...
- Fleet and Monitor automatically refresh configured peers while observed. Address-range scanning is reserved for explicit provisioning or recovery discovery.
...
- Fleet inventory reads the gateway-owned peer cache and, while Fleet or Monitor is visible, sends one bounded encrypted maintenance request to one configured peer every ten seconds.
...
- The gateway does not run a headless maintenance or diagnostic sweep. Relay/input control owns LoRa airtime; observed Fleet/Monitor inventory enrichment is low-priority, stale-tolerant, paced, and stops when the observer disappears.
...
- Direct maintenance and operational-poll replies wait 120 ms after request reception so the gateway is back in receive mode. Flasher advances one configured-peer inventory cursor without bursts or parallel requests; delayed work waits for the next ordinary interval.
...
- Password-gated commands: `get_config`, `set_config`, `get_display_name`, `set_display_name`, `factory_reset`, `configure_gateway`, `set_gateway_targets`, `start_discovery`, `provision_all`, `cancel_provisioning`, `wifi_scan`, `configure_wifi`, `provision_fleet_wifi`, `identify`, `remote_identify`, `remote_identify_status`, `reboot`, `ota_pull`, `remote_ota_pull`, `remote_ota_status`, `remote_factory_reset`, `remote_factory_reset_status`, `start_lora_inventory`, `lora_inventory_status`, and `cancel_lora_inventory`.
...
- Gateway display names are presentation metadata, not device configuration. `get_display_name` and `set_display_name` accept one `chip_id` and operate only on the gateway itself or a currently configured peer. Names are blank by default, limited to 16 characters (`A-Z`, `a-z`, `0-9`, space, `_`, `-`, `.`), and a blank set clears one name. Both commands use the existing correlated serial/MQTT admin path and never send LoRa traffic.
- Names live in a gateway-only fixed-record LittleFS sidecar (`/display_names.dat`, plus `.new` and `.bak` recovery copies). The file is checksummed and validated independently of `Settings`; firmware keeps no name cache and adds no name field to remote or whole-fleet payloads. Forget and confirmed full remote reset clear a peer name only after peer configuration saves. Readdress, OTA, reboot, and reset-keep-in-fleet preserve it by chip ID; gateway factory reset clears the sidecar.
...
- `remote_ota_status` is a password-gated admin command requiring `addr`. It returns that remote's compact OTA handoff record: destination address, transfer ID, numeric stage code, string stage name (`idle`, `sending`, `awaiting_ack`, `accepted`, `unconfirmed`, or `failed`), error code, and timestamp. The gateway has one RF manifest slot but retains one fixed status record per configured address, allowing accepted remotes to download concurrently without losing late failure correlation.
- `remote_factory_reset` queues one correlated reset transaction and returns its `transaction_id`. `remote_factory_reset_status` requires the target address and returns the transaction ID, current stage (`idle`, `sending`, `awaiting_ack`, `confirming`, `committed`, `unconfirmed`, or `failed`), and compact error code. The gateway retains the peer unless a matching full-reset confirmation and peer-record save both succeed.
...
- `remote_identify` requires `addr` (or `address`) and accepts an optional `duration_ms` clamped to 1–30 seconds. It queues the same LED pattern on one remote and returns the effective duration plus a transaction ID. `remote_identify_status` requires the target address and reports that transaction's ID and stage (`sending`, `awaiting_ack`, `confirmed`, `unconfirmed`, or `unavailable_power_save`). Both commands work through the selected gateway over USB serial or authenticated MQTT admin; Flasher polls this single bounded record rather than blocking an admin transport while LoRa acknowledgement is pending.
...
`W` (WiFi provisioning), `E` (correlated factory-reset request), and `F` (factory-reset status) reuse the same encrypted 12-byte payload slot with custom byte layouts via the radio layer raw-payload send path. Legacy fire-and-forget factory reset used `X`; current firmware intentionally does not accept it.
