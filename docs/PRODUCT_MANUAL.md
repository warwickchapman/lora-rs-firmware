...
- Fleet OTA uses the selected gateway's compile-time compatibility profile (`433_za` or `915_us`) to choose a release asset. A missing profile is displayed as unavailable and never guessed; the Flash workflow retains a manual Region choice for blank or unreachable devices.
- Fleet's Name column stores optional gateway and remote display names on the selected gateway. Editing is inline and gateway-local; names are keyed by physical chip ID, create no LoRa traffic, and remain separate from remote operational configuration.
