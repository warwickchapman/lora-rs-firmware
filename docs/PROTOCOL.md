...
- Durable Transport Specifications:
  ...
  - **Settings Patch Contract**: Retained `config/#` topics are the MQTT Settings read source for every non-secret field. MQTT `get_config` remains redacted and diagnostic-only. Authenticated MQTT `set_config` accepts registry-authorized partial patches, including typed secret replacements; it never reads secrets back or retains them. Clients must diff against the retained snapshot, keep each command below the packet budget, and split large changes into sequential patches. WiFi, MQTT connection/topic-root, and reboot-triggering fields must be grouped in the final patch because applying them can end the current admin transport. Full-settings JSON documents are prohibited on MQTT.
