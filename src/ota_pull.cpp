...
bool fetchSha256(const char *url, uint8_t digest[32], String &error) {
  WiFiClient client;
  HTTPClient http;
  ...
  if (readTotal != static_cast<size_t>(contentLength)) {
...
bool otaPullFromUrl(const char *url, const char *sha256Hex, String &error) {
  ...
  if (sha256Hex != nullptr && sha256Hex[0] != '\0') {
    uint8_t digest[32];
    if (!fetchSha256(url, digest, error)) return false;
