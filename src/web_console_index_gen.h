...
async function uploadOta() {
  ...
  const resetAfterUpdate = !!(document.getElementById('ota_factory_reset_after') && document.getElementById('ota_factory_reset_after').checked);
  ...
  fd.append('factory_reset_after_update', resetAfterUpdate ? '1' : '0');
  try {
    const res = await fetch('/api/ota', { method: 'POST', body: fd });
    const t = await res.text();
    if (!res.ok) { if (el) el.innerText = `OTA failed: ${t}`; return; }
    if (el) el.innerText = resetAfterUpdate ? 'OTA complete, reset queued, rebooting...' : 'OTA complete, rebooting...';
